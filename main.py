from mining_coin_train_dqn import dqn_tain_model
...
#         self.policy_net, self.target_net = Net(), Net()                           # 利用Net创建两个神经网络: 评估网络和目标网络
#         self.learn_step_counter = 0                                             # for target updating
#         self.memory_counter = 0                                                 # for storing memory
#         self.memory = np.zeros((MEMORY_CAPACITY, N_STATES * 2 + 2))             # 初始化记忆库，一行代表一个transition
#         self.optimizer = torch.optim.Adam(self.policy_net.parameters(), lr=LR)    # 使用Adam优化器 (输入为评估网络的参数和学习率)
#         self.loss_func = nn.MSELoss()                                           # 使用均方损失函数 (loss(xi, yi)=(xi-yi)^2)
#
#     def choose_action(self, x):                                                 # 定义动作选择函数 (x为状态)
#         x = torch.unsqueeze(torch.FloatTensor(x), 0)                            # 将x转换成32-bit floating point形式，并在dim=0增加维数为1的维度
#         if np.random.uniform() < EPSILON:                                       # 生成一个在[0, 1)内的随机数，如果小于EPSILON，选择最优动作
#             actions_value = self.policy_net.forward(x)                            # 通过对评估网络输入状态x，前向传播获得动作值
...
#             self.target_net.load_state_dict(self.policy_net.state_dict())         # 将评估网络的参数赋给目标网络
...
#         q_eval = self.policy_net(b_s).gather(1, b_a)
